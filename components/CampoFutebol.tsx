...
function CampoSVG() {
  ...
  return (
    ...
    >
      ...
      <path
        d="M 23 16 A 11 11 0 0 1 12 27"
        fill="none"
        stroke={s}
        strokeWidth="1.5"
      />
      <path
        d="M 277 16 A 11 11 0 0 0 288 27"
        fill="none"
        stroke={s}
        strokeWidth="1.5"
      />
      <path
        d="M 12 423 A 11 11 0 0 0 23 434"
        fill="none"
        stroke={s}
        strokeWidth="1.5"
      />
      <path
        d="M 288 423 A 11 11 0 0 1 277 434"
        fill="none"
        stroke={s}
        strokeWidth="1.5"
      />
...
const CDN =
  "https://cdn.jsdelivr.net/gh/Ian-costermani/brasileirao-fantasy@master/static";
...
function abreviarNome(nome: string): string {
  ...
  const sobrenome = significativas.length > 0
    ? significativas[significativas.length - 1]
    : partes[partes.length - 1];
  return `${inicial}. ${sobrenome.substring(0, 8).toUpperCase()}`;
}

function PlayerCard(
  { jogador, modoAoVivo, fraco, corTime }: {
    jogador: Jogador;
    modoAoVivo: boolean;
    fraco: boolean;
    corTime: string;
  ...
  const escudoSrc = jogador.clube
    ? `${CDN}/escudos/${toSlug(jogador.clube)}.jpg`
    : null;
      ...
      <div class="campo-player-card">
        <div class="campo-player-foto-wrap">
          ...
          <img
            ...
            onError={() => {
              if (fotoSrc === `${CDN}/players/${slugNome}.webp`) {
                setFotoSrc(
                  `${CDN}/players/${slugNome}.jpg`,
                );
              } else if (
                fotoSrc === `${CDN}/players/${slugNome}.jpg`
              ) setFotoSrc(`${CDN}/players/${slugNome}.png`);
              else if (
                fotoSrc === `${CDN}/players/${slugNome}.png` && slugClube
              ) setFotoSrc(`${CDN}/players/${slugNome}-${slugClube}.webp`);
              else if (
                slugClube &&
                fotoSrc === `${CDN}/players/${slugNome}-${slugClube}.webp`
              ) setFotoSrc(`${CDN}/players/${slugNome}-${slugClube}.jpg`);
              else if (
                slugClube &&
                fotoSrc === `${CDN}/players/${slugNome}-${slugClube}.jpg`
              ) setFotoSrc(`${CDN}/players/${slugNome}-${slugClube}.png`);
            }}
          />
          {escudoSrc && (
            <img
          ...
          {sInfo && (
            <span class="campo-status-sym" style={`color:${sInfo.cor}`}>
              {sInfo.sym}
            </span>
          )}
        </div>
        <div class="campo-player-box" style={neonStyle}>
          ...
          {modoAoVivo
            ...
            : (
              <span class="campo-player-pos">
                {POSICAO_ABREV[jogador.posicao] ?? jogador.posicao}
              </span>
            )}
        </div>
      </div>
...
export function CampoFutebol(
  { jogadores, modoAoVivo = false, corTime = "#ffffff", campoBg, escudo }:
    Props,
) {
